...
// Major Indian cities coordinates for visualization
const indianCities = [
...
export default function EmotionMap() {
  ...
  const getDominantEmotion = (stats: any) => {
    ...
    const emotions = { happy: stats.happy, angry: stats.angry, neutral: stats.neutral, suggestion: stats.suggestion };
  ...
  const getIntensity = (stats: any) => {
    ...
    const total = stats.happy + stats.angry + stats.neutral + stats.suggestion;
    return Math.min(total / 20, 1); // Normalize to 0-1, scale based on total votes
  };

  // Generate city data with some variation based on actual vote stats
  const generateCityData = (stats: any) => {
    if (!stats) return [];
    
    return indianCities.map(city => {
      // Add some random variation to make cities look different while keeping overall sentiment
      const variation = () => Math.floor(Math.random() * 10) - 5; // -5 to +5 variation
      
      return {
        ...city,
        happy: Math.max(0, stats.happy + variation()),
        angry: Math.max(0, stats.angry + variation()),
        neutral: Math.max(0, stats.neutral + variation()),
        suggestion: Math.max(0, stats.suggestion + variation()),
      };
    });
  };
  ...
  useEffect(() => {
    if (!mapRef.current || !voteStats || isLoadingStats) return;
    ...
    // Generate city data based on real voting stats
    const cityData = generateCityData(voteStats.stats);

    // Add emotion clouds for each city
    cityData.forEach(city => {
      const dominant = getDominantEmotion(city);
      const intensity = getIntensity(voteStats.stats);
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-16 bg-muted rounded-full h-2">
                          <div 
                            ...
                            style={{ width: `${selectedCity.happy}%` }}
                          />
                        </div>
                        <span className="text-sm font-medium">{selectedCity.happy}%</span>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-16 bg-muted rounded-full h-2">
                          <div 
                            ...
                            style={{ width: `${selectedCity.angry}%` }}
                          />
                        </div>
                        <span className="text-sm font-medium">{selectedCity.angry}%</span>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-16 bg-muted rounded-full h-2">
                          <div 
                            ...
                            style={{ width: `${selectedCity.neutral}%` }}
                          />
                        </div>
                        <span className="text-sm font-medium">{selectedCity.neutral}%</span>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-16 bg-muted rounded-full h-2">
                          <div 
                            ...
                            style={{ width: `${selectedCity.suggestion}%` }}
                          />
                        </div>
                        <span className="text-sm font-medium">{selectedCity.suggestion}%</span>
                      </div>
                    </div>
                  </div>
